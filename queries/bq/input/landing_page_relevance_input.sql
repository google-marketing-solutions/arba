...
-- Fetches mapping between ad_group, urls, keywords and ads.

-- @param target_dataset BigQuery dataset where output result is stored.
...
CREATE OR REPLACE TABLE `{target_dataset}.landing_page_relevance_input` AS (
  WITH
    TopAdGroups AS (
      SELECT
        ad_group_id,
  ...
  SELECT
    L.ad_group_id,
    L.url,
    IFNULL(ARRAY_TO_STRING(K.keywords, '|'), '') AS keywords,
    IFNULL(ARRAY_TO_STRING(ARRAY_CONCAT(A.headlines, A.descriptions), '|'), '') AS ads
  FROM Landings AS L
