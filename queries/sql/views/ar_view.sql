...
CREATE OR REPLACE TABLE `{target_dataset}.ar_view` AS (
  ...
  SELECT
    ...
    KV.ad_group_name,
    CASE
      WHEN KV.historical_creative_quality_score = "UNSPECIFIED" THEN 0
      WHEN KV.historical_creative_quality_score = "BELOW_AVERAGE" THEN 1
      WHEN KV.historical_creative_quality_score = "AVERAGE" THEN 2
      WHEN KV.historical_creative_quality_score = "ABOVE_AVERAGE" THEN 3
    END AS ar,
